...
*
*/
gost_ctx  g_cipher_context;

const gost_subst_block * g_p_substitution_block = 0;

/******************************************************************************
*
*/

unsigned int nv_GOST_28147_89_crypt_( 
  const byte* p_key,
  char          sz_key, 
  char*         p_data_clear,
  char*         p_data_cipher,
  ...
  int         is_decrypt )
{
  unsigned int n = sz_data;
  char  mas_key[32];

  if( sz_data == 0 ) 
    return 0;

  if( !p_data_clear || !p_data_cipher )
  ...
  if( sz_key )
    memcpy( mas_key, p_key, sz_key );

  //

  if( !g_p_substitution_block )
  {
    g_p_substitution_block = ( const gost_subst_block * )&Gost28147_CryptoProParamSetA;

    kboxinit( ( gost_ctx * )&g_cipher_context, g_p_substitution_block );
  }

  gost_key( ( gost_ctx * )&g_cipher_context, mas_key );
  ...
  {
    if( is_decrypt )
      gostdecrypt ( ( gost_ctx * )&g_cipher_context, p_data_cipher, p_data_clear  );
    else
      gostcrypt   ( ( gost_ctx * )&g_cipher_context, p_data_clear, p_data_cipher );
...
unsigned int nv_GOST_28147_89_encrypt( const char* p_key, char sz_key, const char* p_data_in, char* p_data_out, unsigned int sz_data )
{
  return nv_GOST_28147_89_crypt_( p_key, sz_key, (char*)p_data_in, p_data_out, sz_data, 0 );
}
...
unsigned int nv_GOST_28147_89_decrypt( const char* p_key, char sz_key, const char* p_data_in, char* p_data_out, unsigned int sz_data )
{
  return nv_GOST_28147_89_crypt_( p_key, sz_key, p_data_out, (char*)p_data_in, sz_data, 1 );
}
